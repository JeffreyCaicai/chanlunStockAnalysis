...
class WebTests(unittest.TestCase):
    ...
    def test_root_serves_html(self):
        ...
        self.assertIn('class="tab-controls"', body)
        self.assertIn('id="analysisTabButton"', body)
        self.assertIn('id="backtestTabButton"', body)
        self.assertIn('data-tab-target="analysisTab"', body)
        self.assertIn('data-tab-target="backtestTab"', body)
        self.assertIn('id="analysisTab" class="tab-panel"', body)
        self.assertIn('id="backtestTab" class="tab-panel" hidden', body)
        self.assertIn("showWorkbenchTab", body)
        self.assertIn('button.setAttribute("aria-selected"', body)
        self.assertIn('panel.hidden = panel.id !== targetId', body)
        self.assertIn('<button id="analyzeButton" onclick="analyze()">运行分析</button>', body)
        self.assertIn('id="analysisProgress"', body)
        self.assertIn('id="analysisProgressText"', body)
        self.assertIn('aria-live="polite"', body)
        self.assertIn('aria-busy="false"', body)
        self.assertIn("@keyframes progress-slide", body)
        self.assertIn("setAnalyzeLoading", body)
        self.assertIn("button.disabled = isLoading", body)
        self.assertIn("progress.hidden = !isLoading", body)
        self.assertIn("setAnalyzeLoading(true", body)
        self.assertIn("setAnalyzeLoading(false", body)
        ...
        self.assertIn('showWorkbenchTab("backtestTab")', body)
